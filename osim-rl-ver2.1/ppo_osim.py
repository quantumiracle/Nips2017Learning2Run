...
class PPO(object):
    ...
    def _build_anet(self, name, trainable):
        with tf.variable_scope(name):
            ...
            mu = tf.layers.dense(l3, A_DIM, tf.nn.relu6, trainable=trainable) # action range [0,1]
            # sigma = tf.layers.dense(l1, A_DIM, tf.nn.softplus, trainable=trainable) # softplus to make it positive
            sigma = tf.layers.dense(l3, A_DIM, tf.nn.sigmoid, trainable=trainable) # softplus to make it positive
...
if args.train:
    ...
    for ep in range(EP_MAX):
        ...
        for t in range(EP_LEN):    # in one episode
            a = ppo.choose_action(np.array(s))
